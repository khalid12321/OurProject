...
# NEW: track "red state" so snapshot happens once on transition to red
was_red_last_frame = {}  # track_id -> bool
...
def save_snapshot(frame, track_id, violation_type, lane_num=None, bbox=None):
    """
    Save full-frame snapshot with:
    - red box around the violated car
    - real-time clock timestamp text
    - write row in violations.db
    """
    ...
    # Red box on violated car in saved image
    if bbox is not None:
        x1, y1, x2, y2 = bbox
...
while cap.isOpened():
    ...
    # Track which IDs exist this frame (for cleanup)
    seen_ids = set(track_ids)

    # 3) Draw bounding boxes and labels + SNAPSHOT ON RED TRANSITION ONLY
    for box, track_id in zip(boxes, track_ids):
        x1, y1, x2, y2 = map(int, box)
        ...
        # ===== SNAPSHOT ONLY WHEN IT BECOMES RED (transition) =====
        is_red = (color[0] == 0 and color[1] == 0 and color[2] == 255)
        prev_red = was_red_last_frame.get(track_id, False)

        if is_red and (not prev_red):
            lane_num_for_db = (lane_of_car[track_id] + 1) if track_id in lane_of_car else None
            save_snapshot(frame, track_id, label, lane_num=lane_num_for_db, bbox=(x1, y1, x2, y2))

        was_red_last_frame[track_id] = is_red
        # ==========================================================
    ...
    # Optional cleanup: remove states for disappeared IDs (keeps dict small)
    for tid in list(was_red_last_frame.keys()):
        if tid not in seen_ids:
            was_red_last_frame.pop(tid, None)
